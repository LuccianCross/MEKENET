...
Reads transactions for a specific device and computes a human-readable
financial report. Supports pagination.
"""
...
def _build_category_breakdown(txs: List[Dict], tx_type: str) -> List[CategoryBreakdown]:
    ...
    for tx in txs:
        if tx["type"] == tx_type:
            ...
            buckets[cat]["total"] += float(tx["amount"])
            buckets[cat]["count"] += 1
    return [
...
def export_report(
    ...
    query = db.query(Transaction).filter(Transaction.device_id == x_device_id)
    ...
    rows = query.order_by(Transaction.date.desc()).offset(offset).limit(page_size).all()
    ...
    total_income = round(sum(float(t["amount"]) for t in txs if t["type"] == "income"), 2)
    total_expense = round(sum(float(t["amount"]) for t in txs if t["type"] == "expense"), 2)
